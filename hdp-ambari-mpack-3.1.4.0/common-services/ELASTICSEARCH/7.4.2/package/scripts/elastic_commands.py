...
def is_systemd_running(params):
    ...
    rc, out, err = get_user_call_output("pidof systemd", params.elastic_user, is_checked_call=False)
    if rc == 0:
...
def create_elastic_config(params):
    ...
    File(path, owner=params.elastic_user, group=params.elastic_group, content=InlineTemplate(params.sysconfig_template))
