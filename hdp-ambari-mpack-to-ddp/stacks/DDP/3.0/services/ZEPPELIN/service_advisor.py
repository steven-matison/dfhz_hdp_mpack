        ...
        if "ZEPPELIN_INTP_CLASSPATH_OVERRIDES" in line:
          if sac_enabled:
            if line.lstrip().startswith("#"):
              line = "export ZEPPELIN_INTP_CLASSPATH_OVERRIDES=\"{{external_dependency_conf}}:/usr/ddp/current/spark-atlas-connector/*\""
            elif "{{external_dependency_conf}}" in line:
              line = line.replace("{{external_dependency_conf}}", "{{external_dependency_conf}}:/usr/ddp/current/spark-atlas-connector/*")
            else:
              k = line.rfind("\"")
              line = line[:k] + ":/usr/ddp/current/spark-atlas-connector/*\"" + line[k+1:]
          else:
            if ":/usr/ddp/current/spark-atlas-connector" in line:
              line = line.replace(":/usr/ddp/current/spark-atlas-connector/*", "")
            elif "/usr/ddp/current/spark-atlas-connector" in line:
              line = line.replace("/usr/ddp/current/spark-atlas-connector/*", "")
